...
watch([selectedDistrictId, selectedCourier], async ([newDisctrictId, newCourier]) => {
  if (newDisctrictId && newCourier && cartWeight.value > 0) {
    await checkOngkir(newDisctrictId, cartWeight.value, newCourier);
  }
})
            ...
            <div class="flex items-start">
              ...
              <div>
                ...
                <span v-if="cart.product.discount != null || cart.product.discount > 0"
                  class="text-sm line-through text-red-500">{{ moneyFormat(cart.product.price * cart.qty) }}</span>
              </div>
            </div>
          ...
          <div class="mb-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div v-for="courier in couriers" :key="courier.value" class="flex items-center">
                <input type="radio" name="courier" :id="`courier-${courier.value}`" :value="courier.value" v-model="selectedCourier" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
